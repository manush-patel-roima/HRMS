...
//         className="border-2 rounded p-2 w-full mb-2 font-semibold"
//         onChange={e => {
...
const OrgChart = () => {
  ...
  const {data: employees=[]} = useQuery({
    queryKey: ['employees'],
    queryFn: () => OrgChartService.getAllEmployees().then(res => res.data)
  })
  ...
  const {data: chartData, isLoading, isFetching} = useQuery({
    ...
    queryFn: async() => {
      if(selectedId){
        return await OrgChartService.getOrgChart(selectedId).then(res => res.data);
      }
        
      return await OrgChartService.getMyOrgChart().then(res => res.data);
  ...
  return (
    <>
      <Select
       ...
       value={employees
              .map(e => ({ value: e.employeeId, label: e.employeeName }))
      ...
      {isFetching && <div className="text-center text-sm text-blue-600">Updating...</div>}
     
      <div className="bg-white p-6 rounded shadow flex flex-col items-center">
        
        
        {chartData.managerChain.length > 0 && (
          <div className="relative pb-6 flex flex-col-reverse items-center">
            
            <div className="absolute bottom-0 w-px h-6 bg-gray-300"></div>
            
            {chartData.managerChain.map((m, index) => (
              <div key={m.employeeId}>
                {index < chartData.managerChain.length - 1 && (
                  <div className="relative">
                    <div className="absolute left-1/2 transform -translate-x-1/2 w-px h-6 bg-gray-300"></div>
                    <div className="h-6"></div> 
                  </div>
        ...
        <div className="relative p-6 rounded-lg shadow-lg text-center w-52 z-10 bg-gray-100">
          <Avatar employee={chartData.selectedEmployee} size="w-16 h-16"/>
        </div>
        ...
        {chartData.directReports.length > 0 && (
          <div className="relative pt-6 w-full flex flex-col items-center">
            ...
            <div className="flex gap-6 justify-center flex-wrap"> 
              {chartData.directReports.map(emp => (
                <div 
                  key={emp.employeeId} 
                  className="bg-sky-100 p-6 rounded-lg cursor-pointer hover:bg-gray-100 text-center w-48 shadow"
